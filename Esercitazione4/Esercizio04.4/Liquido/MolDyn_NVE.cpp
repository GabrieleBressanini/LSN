...
//******************************************************************************************************
//Unità di misura: eV per le energie, atm per la pressione e K per le temperature
//******************************************************************************************************


//******************************************************************************************************
//Ho modificato nstep in 30k, in modo tale da avere 3000 misure e fare 30 blocchi da 100 misure ciascuno
//******************************************************************************************************
...
//******************************************************************************************************
//Qua devo modificare ogni volta che cambio nstep
//******************************************************************************************************
static double potenziale[3000], cinetica[3000], totale[3000], temperatura[3000], pressione[3000];

//LJ units
//Argon
double Ar_m = 39.948;       //massa espressa in u.m.a
double Ar_sigma = 0.34;    //espressa in nanometri
double Ar_eps = 120.;       //epsilon fratto costante di Boltzmann, espressa in Kelvin

//Krypto
double Kr_m = 83.798;       //massa espressa in u.m.a
double Kr_sigma = 0.364;    //espressa in nanometri
double Kr_eps = 164.;       //epsilon fratto costante di Boltzmann, espressa in Kelvin

double kB = 8.6173303/(100000.);  //costante di Boltzmann, espressa in eV/K
...
void Measure(){ //Properties measurement
  ...
  Etot.open("output_etot.dat",ios::app);
    ...
    ofstream EpotaveAr, EkinaveAr, EtotaveAr, TempaveAr, PressaveAr;
    ofstream EpotaveKr, EkinaveKr, EtotaveKr, TempaveKr, PressaveKr;
    
    EpotaveAr.open("ave_epotAr.dat");
    EkinaveAr.open("ave_ekinAr.dat");
    TempaveAr.open("ave_tempAr.dat");
    EtotaveAr.open("ave_etotAr.dat");
    PressaveAr.open("ave_pressAr.dat");
    
    EpotaveKr.open("ave_epotKr.dat");
    EkinaveKr.open("ave_ekinKr.dat");
    TempaveKr.open("ave_tempKr.dat");
    EtotaveKr.open("ave_etotKr.dat");
    PressaveKr.open("ave_pressKr.dat");
...
//Pressure
    ...
    for(int i=0; i<npart-1; i++){
        for(int j=i+1; j<npart; j++){
            ...
            //aggiunto il cutoff
            if(dr<rcut){
                stima_press += 1./pow(dr,12) - 0.5 * 1./pow(dr,6);
            }
        }
    }
    
    stima_press = stima_press * 48 /(3*vol*npart) + (rho * stima_temp);
    ...
    //salvo nei vettori
    potenziale[cont] = stima_pot;        //eV
    cinetica[cont] = stima_kin;          //eV
    totale[cont] = stima_etot;           //eV
    temperatura[cont] = stima_temp;      //K
    pressione[cont] = stima_press;       //atm
    ...
    if(cont == nstep/10 - 1){
        ...
        for(int i=0; i<nblocks; i++){
            ...
            EpotaveAr << i <<"   "<< sum_prog[i] * Ar_eps * kB <<"   "<< err_prog[i] * Ar_eps * kB << endl;
            EpotaveKr << i <<"   "<< sum_prog[i] * Kr_eps * kB <<"   "<< err_prog[i] * Kr_eps * kB << endl;
        }
        ...
        for(int i=0; i<nblocks; i++){
            ...
            su2_prog[i] = su2_prog[i]/(i+1);
            err_prog[i] = error(sum_prog[i],su2_prog[i],i);
            EkinaveAr << i <<"   "<< sum_prog[i] * Ar_eps * kB <<"   "<< err_prog[i] * Ar_eps * kB << endl;
            EkinaveKr << i <<"   "<< sum_prog[i] * Kr_eps * kB <<"   "<< err_prog[i] * Kr_eps * kB << endl;
        }
        ...
        for(int i=0; i<nblocks; i++){
            ...
            su2_prog[i] = su2_prog[i]/(i+1);
            err_prog[i] = error(sum_prog[i],su2_prog[i],i);
            EtotaveAr << i <<"   "<< sum_prog[i] * Ar_eps * kB <<"   "<< err_prog[i] * Ar_eps * kB << endl;
            EtotaveKr << i <<"   "<< sum_prog[i] * Kr_eps * kB <<"   "<< err_prog[i] * Kr_eps * kB << endl;
        }
        ...
        for(int i=0; i<nblocks; i++){
            ...
            su2_prog[i] = su2_prog[i]/(i+1);
            err_prog[i] = error(sum_prog[i],su2_prog[i],i);
            TempaveAr << i <<"   "<< sum_prog[i] * Ar_eps  <<"   "<< err_prog[i] * Ar_eps << endl;
            TempaveKr << i <<"   "<< sum_prog[i] * Kr_eps  <<"   "<< err_prog[i] * Kr_eps << endl;
        }
        ...
        //Medie ed errori a blocchi per la pressione
        for(int i=0; i<nblocks; i++){
            sum = 0;
            for(int j=0; j<M; j++){
                sum = sum + pressione[i * M + j];
            }
            ave[i] = sum / double(M);
            av2[i] = ave[i] * ave[i];
        }
        ...
        for(int i=0; i<nblocks; i++){
            ...
            su2_prog[i] = su2_prog[i]/(i+1);
            err_prog[i] = error(sum_prog[i],su2_prog[i],i);
            PressaveAr << i <<"   "<< sum_prog[i] * Ar_eps * 1.3806485279 /(pow(Ar_sigma,3)) * (10000./101325.)  <<"   "<< err_prog[i] * Ar_eps * 1.3806485279 /(pow(Ar_sigma,3)) * (10000./101325.) << endl;
            PressaveKr << i <<"   "<< sum_prog[i] * Kr_eps * 1.3806485279 /(pow(Kr_sigma,3)) * (10000./101325.) <<"   "<< err_prog[i] * Kr_eps * 1.3806485279 /(pow(Kr_sigma,3)) * (10000./101325.) << endl;
        }
