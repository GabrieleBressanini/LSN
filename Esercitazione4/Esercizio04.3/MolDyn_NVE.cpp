...
//NB: NEL CALCOLO DELLA PRESSIONE C'E' IL CUTOFF E LA DIVISIONE PER IL NUMERO DELLE PARTICELLE
...
//Pressure
    ...
    if(cont == nstep/10 - 1){
        ...
        for(int i=0; i<nblocks; i++){
            ...
            su2_prog[i] = su2_prog[i]/(i+1);
            err_prog[i] = error(sum_prog[i],su2_prog[i],i);
            Pressave << i <<"   "<< sum_prog[i] <<"   "<< err_prog[i] << endl;
        }
